...
main() {
    ...
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')
log_message "CPU Usage: $cpu_usage | RAM Usage: $ram_usage | Disk Usage: $disk_usage | Service Status: $service_status"
...
sudo chmod 644 "$LOG_FILE" || { echo "Error: Cannot set permissions"; exit 1; }
